...
class Logger:
    ...
    def vis_fullsize_images(self, netG, global_step, num_images, input_noise):
        """
        Produce full size (single output) visualisations of the G(z) depending on input_noise parameter (fixed or random).

        Args:
            netG (Module): Generator model object for producing images.
            global_step (int): Global step variable for syncing logs.
            num_images (int): The number of images to visualise.
            input_images (str): Type of input noise distribution to use ('fixed' or 'random')
        ...
        with torch.no_grad():
            ...
            if input_noise == "random":
                img_dir = os.path.join(self.log_dir, 'fake_images')
                if not os.path.exists(img_dir):
                    os.makedirs(img_dir)
            ...
            elif input_noise == "fixed":
                img_dir = os.path.join(self.log_dir, 'fixed_fake_images')
                if not os.path.exists(img_dir):
                    os.makedirs(img_dir)
